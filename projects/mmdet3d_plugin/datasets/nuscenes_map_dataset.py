...
import json
from .rasterized import line_geom_to_mask, overlap_filter
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        semantic_masks, instance_masks = self.get_semantic_map(anns_results, self.bev_size, len(self.MAPCLASSES), 3, 36 )
        
        example['instance_masks'] = DC(instance_masks, cpu_only=False)
        example['semantic_masks'] = DC(semantic_masks, cpu_only=False)

        return example

    def get_semantic_map(self, anns_results, bev_size,  num_classes, thickness,  angle_class ):
        
        gt_vector_instance_list = anns_results['gt_vecs_pts_loc'].instance_list
        gt_vecs_label_list = anns_results['gt_vecs_label']
        confidence_levels = [-1]
        vector_num_list = {}
        for i in range(num_classes):
            vector_num_list[i] = []
        
        for i in range(len(gt_vecs_label_list)):
            line_coords = np.array(gt_vector_instance_list[i].coords)
            if line_coords.shape[0]>=2:
                vector_num_list[gt_vecs_label_list[i]].append(LineString(line_coords))
        local_box = (0.0, 0.0, int(bev_size[0]*0.3), int(bev_size[1]*0.3))
        idx = 1
        filter_masks = []
        instance_masks = []
        forward_masks = []
        ...
        for i in range(num_classes):
            ...
            #distance_transform = cv2.distanceTransform(map_mask, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
            #distance_transform[np.where(distance_transform>=2.5)] =2.5
            #normalized_distance_transform = cv2.normalize(distance_transform, None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            #dis_masks.append(normalized_distance_transform)

            #sobel_x = cv2.Sobel(normalized_distance_transform, cv2.CV_64F, 1, 0, ksize=5)
            #sobel_y = cv2.Sobel(normalized_distance_transform, cv2.CV_64F, 0, 1, ksize=5)
            #orientation_map = np.arctan2(sobel_y, sobel_x)
            #orientation_map_normalized = cv2.normalize(orientation_map, None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        ...
        #dis_masks = np.stack(dis_masks)
        #dis_masks = np.max(dis_masks, axis=0)
        #dis_masks = torch.tensor(dis_masks)
        ...
        semantic_masks = torch.tensor(instance_masks)!=0
        #dis_masks = torch.cat([dis_masks.unsqueeze(0), (~torch.any(semantic_masks, axis=0)).unsqueeze(0)])
  
        #semantic_masks = torch.cat([ (~torch.any(semantic_masks, axis=0)).unsqueeze(0),semantic_masks])
        instance_masks = np.max(instance_masks,axis=0)
        num_instances = instance_masks.max()
        instance_masks_r = np.zeros((num_instances, instance_masks.shape[-2],  instance_masks.shape[-1]))
        for count in range(1, num_instances+1):
            instance_masks_r[count-1]=np.where(instance_masks==count,1,0)
        
        instance_masks_r=np.pad(instance_masks_r,[(0,50-num_instances),(0,0),(0,0)], mode='constant')
        instance_masks_r = torch.tensor(instance_masks_r)
        semantic_masks = torch.cat([torch.any(semantic_masks, axis=0).unsqueeze(0),(~torch.any(semantic_masks, axis=0)).unsqueeze(0)])
        #instance_masks = torch.flip(instance_masks.float(),dims=[2])
        semantic_masks = torch.flip(semantic_masks.float(), dims=[2])
        instance_masks_r = torch.flip(instance_masks_r, dims=[2])
        #return torch.flip(dis_masks.float(), dims=[1])
        return semantic_masks, instance_masks_r
    ...
    def prepare_train_data(self, index):
        ...
        #input_dict['instance_masks']=[]
        input_dict['semantic_masks']=[]
        input_dict['instance_masks']=[]
...
def output_to_vecs(detection):
    ...
    #if 'valid_3d' in detection.keys():
    #    valid_scores = detection['valid_3d'].numpy()
