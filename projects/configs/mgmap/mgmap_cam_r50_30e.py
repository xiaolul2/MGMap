...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        use_rego=True,
        transformer=dict(
            type='MapPerceptionTransformer_1',
        ...
        loss_fn =dict(type='SimpleLoss',pos_weight=4.0, loss_weight=1.),
        loss_dice = dict(type='DiceLoss',loss_weight=15),
        #loss_ins = dict(type='DiceLoss',loss_weight=1.)),
        loss_fn_ =dict(type='SimpleLoss',pos_weight=1., loss_weight=1.),
        loss_dice_ = dict(type='DiceLoss',loss_weight=2.)),
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img','semantic_masks','instance_masks'])
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=pkl_root + 'nuscenes_infos_temporal_train.pkl',
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
             ann_file=pkl_root + 'nuscenes_infos_temporal_val.pkl',
             map_ann_file=pkl_root + 'nuscenes_map_anns_val.json',
