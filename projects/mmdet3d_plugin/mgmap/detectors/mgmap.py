...
class MGMap(MVXTwoStageDetector):
    """MGMap.
    Args:
    ...
    def forward_pts_train(self,
        ...
        """
        #add sad
        # outs,loss_sad = self.pts_bbox_head(
        #     pts_feats, lidar_feat, img_metas, prev_bev)
        #pts_feat是个长度为1的列表[tensor([[[[[-4.4189e...ackwards>)]
        #pts_feats[0].shape
        #torch.Size([2, 6, 256, 15, 25])
        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, img_metas, prev_bev)
        #semantic_masks=None
        #add sad
        # loss_inputs = [gt_bboxes_3d, gt_labels_3d, semantic_masks, loss_sad,outs]
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, semantic_masks, instance_masks,outs]
    ...
    def forward_train(self,
        ...
        # img_feats = None#self.extract_feat(img=img, img_metas=img_metas)
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        losses = dict()
        #
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            gt_labels_3d, semantic_masks, instance_masks,img_metas,
    ...
    def simple_test_pts(self, x, lidar_feat, img_metas, prev_bev=None, rescale=False):
        """Test function"""
     #   add sad
        # outs,loss_sad= self.pts_bbox_head(x, lidar_feat, img_metas, prev_bev=prev_bev)
