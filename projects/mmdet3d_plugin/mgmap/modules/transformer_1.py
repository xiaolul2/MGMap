...
from .bevencode_multi import BevEncode_1
from .mlp import MLP
...
class MapPerceptionTransformer_1(BaseModule):
    ...
    def __init__(self,
                 ...
                 modality='vision',
                 **kwargs):
        super(MapPerceptionTransformer_1, self).__init__(**kwargs)
        if modality == 'fusion':
        ...
        self.bevencode = BevEncode_1(inC=256, outC=2)
    ...
    def attn_bev_encode(
        ...
        #    print(each)#['can_bus'])#, each['can_bus'].shape)
    ...
    def forward(self,
        ...
        #    lidar_feat, multi_bev_feats = self.bevencode(lidar_feat)

        dtype = lidar_feat.dtype
        ##ins_embed = self.ins_embedded(lidar_feat) #bs 50 H W 

        
        scales = len(multi_bev_feats)
        bs, feat_dim, ori_H,ori_W= lidar_feat.shape

        bev_embed_list = []
        spatial_shapes=[]
        for bev_feats in multi_bev_feats:
            bs, feat_dim, tmp_H,tmp_W = bev_feats.shape
            
            bev_feats = bev_feats.reshape(bs,feat_dim,tmp_H*tmp_W)
        
        
            bev_feats = bev_feats.permute(2,0,1)  #bs 2k 256
            bev_embed_list.append(bev_feats)
            spatial_shapes.append([tmp_H,tmp_W])
        bev_embeds = torch.cat(bev_embed_list, 0)
                                                               
        query_pos, query = torch.split(
            object_query_embed, self.embed_dims, dim=-1)
        query_pos = query_pos.unsqueeze(0).expand(bs, -1, -1)
        query = query.unsqueeze(0).expand(bs, -1, -1)#.view(bs, 50,20, query.shape[-1])
        ...
        inter_states, inter_references, inter_ins = self.decoder(
            query=query,
            key=None,
            value=bev_embeds,
            query_pos=query_pos,
            ...
            multi_bev= multi_bev_feats,
            reg_branches=reg_branches,
            cls_branches=cls_branches,
            #valid_branches=valid_branches,
            spatial_shapes=spatial_shapes,#torch.tensor([[bev_h, bev_w]], device=query.device),
            level_start_index=torch.tensor([0, 5000, 6250, 6575], device=query.device),
            # level_start_index=torch.tensor([0], device=query.device),
            **kwargs)

        inter_references_out = inter_references

        return lidar_feat, multi_bev_feats, inter_states, init_reference_out, inter_references_out, inter_ins 