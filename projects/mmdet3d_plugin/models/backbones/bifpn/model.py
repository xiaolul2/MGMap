...
class BiFPNLayer(nn.Module):
    ...
    def _forward_fast_attention(self, inputs):
        ...
        p6_up = self.conv6_up.forward(self.swish(weight[0] * p6_in + weight[1] * F.interpolate(p7_in,size = p6_in.size()[2:], mode='bilinear', align_corners=True)))

        # Weights for P5_0 and P6_1 to P5_1
        p5_w1 = self.p5_w1_relu(self.p5_w1)
        weight = p5_w1 / (torch.sum(p5_w1, dim=0) + self.epsilon)
        # Connections for P5_0 and P6_1 to P5_1 respectively
        p5_up = self.conv5_up.forward(self.swish(weight[0] * p5_in + weight[1] * F.interpolate(p6_up,size = p5_in.size()[2:], mode='bilinear', align_corners=True)))
...
class BiFPN(nn.Module):
    def __init__(
        ...
        self.final_conv = nn.Conv2d(600,256, kernel_size=3, padding=1, bias=False)
